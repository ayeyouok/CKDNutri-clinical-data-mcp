...
#   - P4 rules.json 等级（P3-5 修正 2026-08-18，此前注释误写 R-02 为 L1）：
#     R-02 高钾血症 K>5.5 即 **L2**（需尽快处理），R-03 危急高钾 K>6.5 即 **L1**
#     （最高风险）——5.5-6.5 的高钾属"需尽快处理"的风险事件，但未到危急值。
# 因此同一数值（如 K=6.0）：P1 家长侧显示"偏高/非危急"，P4 医生侧判 L2 风险——
